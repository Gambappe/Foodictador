...
**D-8 — Declared settings are durable; experiences live in XTrace; a recommendation reads three inputs.** Decided by the product owner, after D-7 and M11 had established that XTrace cannot round-trip either a read or a settings record.

The split is by **who authored the fact and what it is for**, not by tier:

| input | what it is | where it lives | why |
| --- | --- | --- | --- |
| **declared settings** | what the user states about themselves — allergies, intolerances, budget band, portion, solo comfort, off-limits topics | a **durable store** (the relay's settings namespace; P0.8 made it atomic) | the user asserted these and expects them honoured exactly. `~11/16` non-deterministic retention is not a quality question here: a dropped allergy is a blocked confession written to the pool, or a card recommending something unsafe |
| **the user's experiences** | their confessions, in their own words | **XTrace**, personal scope | prose in, prose out — the one thing §14 measured XTrace as strong at (8/8 raw prose). The value is the *synthesis*, not verbatim recall, so extraction is the feature rather than a lossy detour |
| **the group's experiences** | pooled reads, rendered as prose | **XTrace**, `confit:pool` | cross-record induction, the §8 differentiator. Landed in M12/M14 |

**An Ask must consume all three.** Today it consumes one and a half: settings (broken — M11) and the group claim (working — M12/M14). **The personal scope is written and never read.** `grep` finds no query against a profile scope for synthesis anywhere; `writeProse` has no reader. So the middle column of the architecture above is not degraded, it is absent, and the confession prose has been pure cost since it was written.

Consequences:

- **`NarratorFacts.inducedClaim` is singular and now has two callers.** The personal claim ("what *you* keep doing") and the group claim ("what *people like you* keep doing") are different sentences making different promises, and collapsing them onto one field would let one silently replace the other. Contract change; registered as **M16**.
- **`writeProse`'s durability story does not survive this decision.** If a confession is the *only* copy of an experience, then `~11/16` retention loses about three in ten, and it cannot even be detected: `verifyPendingProse` clears an item when `jobStatus` is `complete`, but `complete` means *the extraction job finished*, not *the memory is retrievable* — which is exactly the case retention drops. The guide's R5 prescribes polling until retrievable and re-ingesting, and that is **impossible in a CLI today** because the pending queue is an in-process array that "dies with the process" (M3's own docblock) and `confess` exits immediately. Needs a durable spool that drains on confirmed *retrievability*, not on job completion. Registered as **M17**. Note this is a spool, not a second permanent copy — it is deleted once XTrace demonstrably holds the text, which keeps the confession's home in XTrace as this decision requires.
- **L4 gets more important, not less.** Two substrate-authored claims will reach the card instead of one, and nothing lints either.
...
Build: an over-budget place survives `exclusionsFor` when a **citable** cohort — `k >= KFLOOR`, on a driver this user's `UsualProfile` evidences — includes that place in its `placeIds`. The declaration holds until evidence contradicts it, and the gate reuses K6's `matched()` and the privacy threshold the product already trusts: five strangers must have signalled about that place before the wall moves. Two properties are stated here rather than left to be discovered. Polarity is deliberately not consulted: the lift stops the wall from *hiding* evidence, and the pool term still weighs that evidence's sign, so a place lifted on five regrets ranks like a place five people regret rather than being endorsed by its own exclusion. And `giConstraint` stays hard regardless — it is the medical constraint D-8/D-9 already treat as non-negotiable, not a preference. Cost taken: `exclusionsFor`/`candidates` now take `reads` — still pure, still no clock. The owner may overrule toward (b) for the demo; the cost of unwinding is deleting one helper and narrowing two signatures.

**D-13 — Batched induction beats per-read deletion handles in the sweeper's backfill.** Decided by the product owner, on the tradeoff the change surfaced.

The sweeper re-ingested one read per call, deliberately: its note said batching "would group unrelated reads that happened to fail together." That objection was already answered — grouping is M2's, by driver and chunked at 20, so a conversation is one cohort's worth of related reads and not an arbitrary pile. What per-read actually produced, measured on a live pass: **219 separate ingests, 219 single-read conversations**, each yielding an episode that can only paraphrase itself — the exact register M15 had to abandon a whole scope to escape — and the burst that tripped SL-58's 429. Batched: 220 reads as 17 conversations in 3 seconds.

The cost is real and the owner accepted it knowingly: **a batched read has no per-read ingest ledger.** `memories_created` for a batched job covers every read in it and says which read produced which memory nowhere, so M10's attribution is unavailable and `forget` reports `skipped` for those reads. Attributing them by matching extracted text is the fuzzy deletion M10's own note forbids, and is not an option that was declined lightly — it is not an option at all.

So `forget`'s promise is now explicitly weaker for reads that reached XTrace via a batch — seeded reads and backfilled ones — and unchanged for reads pooled by a live `confess`, which is still one read per call because a live confession *is* one read. That asymmetry should be said out loud if anyone asks what `forget` deletes.

The second cost: the backfill is all-or-nothing, so a `429` loses the batch rather than one read. Accepted because the sweeper retries every unconfirmed read on the next pass — a lost batch costs minutes, where 219 single-read conversations cost a scope.

**The general rule, since this is the second time it has come up (see D-10):** a design note that forbids something is only as good as the measurement behind it. This one forbade batching on a hypothetical about grouping, and the hypothetical was false by the time it was written.

**D-12 — The relay's open surface serves no ordering; the operator's surface is unchanged (resolves P0.9).** Decided by the claiming session as the de-facto relay owner (P0.4/P0.8 lineage), recorded here per §2, its own commit.

P0.9's brief framed the choice as (a) coarsen `received_at` — breaking the sweeper's seconds-precision settle window and the frozen `RelayEntry` — or (b) token-gate `GET /reads` — breaking P0.4's tested "reads stay open" disclosure posture. Both halves of that dilemma assumed one view per route. The resolution is neither: **the open view coarsens, the authenticated view keeps today's semantics exactly.** Open `GET /reads` serves `{read, received_at}` at day precision, sorted by `read_id` (arrival-independent), without `ingest_job_id`/`pool_memories`, and refuses `?since=` with 401 — an ordering query has no open answer. The same route under M11's `x-relay-token` header serves the pre-P0.9 view byte-for-byte: seconds, arrival order, `since`, full metadata. M4's client sends the header on every request, so the sweeper, PoolView, counting and the frozen contracts do not move. `/stats` follows the same rule: `count` exact on both views, open age day-floored.

The line this draws, said plainly: what D-7 silently added — reconstructing any past day's arrival ordering from a one-shot open read of a store that now holds everything for ever — is closed. What design v0.8 §7 accepted and disclosed — a live poller diffing open snapshots to see arrivals as they happen — remains, because it needs no timestamp to work and closing it means closing the open surface entirely, which is [prod]'s call ("coarse timestamps and opaque cursors", the cursor half staying [prod]). `settings/**` was already token-on-read (M11) and is untouched.
