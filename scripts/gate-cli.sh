...
printf '\n'
printf 'The protocol was rewritten for D-7 (G7): (1) the relay survives a\n'
printf 'SIGKILL losing nothing, (2) deletion deletes by XTrace handle, and\n'
printf '(3) induction yields a claim grounded in the seeded place names.\n'
printf '\n'
printf 'Step 1 runs anywhere -- `npm run gate0` spawns its own relay on a\n'
printf 'scratch snapshot and needs no credentials. Steps 2-3 need\n'
printf 'XTRACE_BASE_URL and XTRACE_API_KEY, and step 3 needs the pool seeded\n'
printf '(`confit pass seed`) and settled.\n'
printf '\n'
printf 'To clear this: run `npm run gate0` in a session holding the XTrace\n'
printf 'credentials and commit the regenerated docs/gate0-results.md. The\n'
printf 'record shows **Status: PASS** only when all three steps pass.\n'
printf '\n'
